...
public class JsonUtil {
    ...
    public static <T, G> T toObjectGeneric(String jsonString, Class<T> mainClazz, Class<G> genericClazz) {
        if (StringUtils.isEmpty(jsonString)) {
            throw new RuntimeException("待反序列化json字符串为空");
        }
        try {
            ...
            throw new RuntimeException("json字符串反序列化失败，jsonString-" + jsonString, e);
        }
    }
    ...
    public static String toString(Object sourceObject) {
        try {
            ...
            throw new RuntimeException("对象序列化失败，sourceObject-" + sourceObject.toString(), e);
        }
    }
